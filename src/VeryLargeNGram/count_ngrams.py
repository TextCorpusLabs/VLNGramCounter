...
def count_ngrams(source: pathlib.Path, dest: pathlib.Path, size: int, control: int, include: pathlib.Path, exclude: pathlib.Path, cutoff:int, top: int, keep_case: bool, keep_punct: bool) -> None:
    """
    ...
    ----------
    source : pathlib.Path
        The folder containing TXT files
    dest : pathlib.Path
        The folder to store the converted CSV file
    size :  int
